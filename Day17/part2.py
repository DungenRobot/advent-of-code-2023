...
def generate_neighbors(row, col, history):
    ...
    for dir_name in directions:
        ...
        if reverse[dir_name] == last_dir: 
            continue
        if (dir_name == last_dir) or (last_dir == '_'):
            if (streak >= 10): 
                continue
        elif (streak < 4): 
            continue
        else:
            long_pos = (row + dir[0] * 4, col + dir[1] * 4)
            long_inside_grid = (0 <= long_pos[0] < len(heat_map)) and (0 <= long_pos[1] < len(heat_map[0]))

            if (not long_inside_grid):
                continue
...
def main():
    ...
    while unsearched != []:
        row, col, history, heat_loss, test_history = unsearched.pop(0)

        if [row, col] == target: 
            minimum_heat = heat_loss
            break
        ...
        for pair in generate_neighbors(row, col, history):
            ...
            new_state = (neighbor_pos[0], neighbor_pos[1], new_history, heat_loss + cost, test_history + dir_name)
